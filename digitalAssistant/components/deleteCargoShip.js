...
module.exports = {
  metadata: () => ({
    name: 'allegro.deleteCargoShip',
    properties: {
        answerInfo: { required: true, type: 'string' },
  ...
  invoke: (conversation, done) => {
    ...
    const { answerInfo } = conversation.properties();
    ...
    request(options, (error, response) => {
      ...
      if (response.statusCode == 200) {
        conversation.logger().info("Successful deleting Cargoship Information");
        
        const _answerInfo = response.body;
        
        conversation.logger().info("Return Information is "+ JSON.stringify(_answerInfo));
     
        conversation.variable(answerInfo,_answerInfo);
